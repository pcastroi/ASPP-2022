% RoexFilterTailPSM2.m - Power spectrum model threshold prediction for notched noise
% experiment assuming single parameter roex filter (roex(p)). Assumes that upper band
% goes to infinity. 
%
% Usage: thres = RoexFilterTailPSM2(g,N0dB,fc,p,k)
%
...
function pw = RoexFilterTailPSM2(g,N0dB,fc,p,k)

N0 = 10^(N0dB/10);			% convert spectral density from dB to power

ps = 2.*k*N0.*fc.*(2/p-exp(-p.*g).*(2+p.*g)/p);
