...
tref2 = bpnoise(def.intervallen,set.lcut,set.ucut,def.samplerate) .* set.window;
tuser = bpnoise(def.intervallen,set.lcut,set.ucut,def.samplerate) .* set.window;

% get the correct levels
tref1 = tref1/rms(tref1) * 10^((set.wishlevel)/20);
tref2 = tref2/rms(tref2) * 10^((set.wishlevel)/20);
