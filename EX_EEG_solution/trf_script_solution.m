...
for trial = 1:2
    ...
    % CODE TO PREPROCESS AUDIO
    % E.G. FILTERING, ENVELOPE EXTRACTION, ETC.
    cutoff = 10; [b,a] = butter(3, cutoff/fsa*2, 'low');
    wav{trial} = abs(hilbert(wav{trial}.^.3));
    wav{trial} = filtfilt(b,a, wav{trial});
    
    stim{trial} = resample(wav{trial},fse,fsa); % resample from audio rate to eeg sampling rate
    
end

load stimproc.mat % linearized speech stimuli processed through a model of the cochlea
...
from  = 1; % use data from sample
to    = length(stim{1}); % and to sample
...
for trial = 1 : Ntrials % each trial
        ...
        % E.g. see help \ or help pinv or ridge
        
        G = (XXT+100*eye(size(XXT)))\XY; % regularized regression
        %G = ridge(y',X',100);
...
load('Gout');
G = Gout;
load('neweeg') % load some unseen eeg
...
% Prediction: on trial 1 the listener attends to talker 2
% Prediction: on trial 2 the listener attends to talker 1
