...
% $Revision: 1.9 $  $Date: 2003/06/13 07:36:29 $
...
phi = -2*pi/a * (t+ ( (tau0-t).^(gamma+1) - tau0^(gamma+1) ) / ...
                    ((beta*exp(alpha*L/2))^gamma * (gamma+1)) );
...
if o_flat,
  amp = sqrt(1/a*(-gamma*exp(-L*alpha/2)/beta* (exp(-L*alpha/2)*(1+(t0-t)/beta)).^(gamma-1)));
  amp = amp / max(abs(amp));
else
  amp = 1;
end
